...
function confirmDeleteA() {
    if (confirm("There are Activities created for this event - Are you sure you wish to delete this Event?") == true) {
        return true;
    } else {
        alert("Event was not deleted!");
        return false;
...
function confirmDeleteD() {
    if (confirm("The End Date for this event has not passed yet - Are you sure you wish to delete this Event?") == true) {
        return true;
    } else {
        alert("Event was not deleted!");
        return false;
...
function confirmDeleteS() {
    if (confirm("Do you wish to sign off the successful completion of this event?") == true) {
        return true;
    } else {
        return false;
    }
...
$countSQL = mysql_query("SELECT * FROM Activities WHERE Event_ID = '$eID'");
$confirmedA = mysql_num_rows($countSQL);
...
if($eEDate > $currentDate) {
    echo '<form action="deleteEventProcess.php?dEvent_ID='.$eID.'" method="post" onsubmit="return confirmDeleteD()">';
} else if ($confirmedA >= 1) {
    echo '<form action="deleteEventProcess.php?dEvent_ID='.$eID.'" method="post" onsubmit="return confirmDeleteA()">';
} else {
    echo '<form action="deleteEventProcess.php?dEvent_ID='.$eID.'" method="post" onsubmit="return confirmDelete()">';
}


?>
<input type="submit" class="redButton" value="Delete" id="deleteButton">
</form>
...
<form action="signOff.php?e_ID=<?php echo $eID; ?>" method="post" id="backButton2" onsubmit="return confirmDeleteS()">
<input type="submit" class="greenButton" value="Sign-Off">
</form>
</div>
